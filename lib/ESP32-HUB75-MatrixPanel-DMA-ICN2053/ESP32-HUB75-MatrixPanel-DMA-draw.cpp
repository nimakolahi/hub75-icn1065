...
void MatrixPanel_DMA::writePixelDMA(int16_t dma_x, int16_t dma_y, uint16_t color)
{
  if (frame_buffer.len == 0) return;
  if (!m_cfg.double_buff) waitDmaReady();
  uint16_t* row = (uint16_t*)&frame_buffer.frameBits[back_buffer_id][dma_y * frame_buffer.row_len];
  row[dma_x] = color;
}
