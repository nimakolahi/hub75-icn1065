...
class MatrixPanel_DMA {
  ...
  public:
    ...
    // Write a pixel directly to the DMA frame buffer (dma_x, dma_y are in DMA space, not physical space).
    // Used by VirtualMatrixPanel after coordinate remapping to bypass the phys_width/phys_height remap.
    void writePixelDMA(int16_t dma_x, int16_t dma_y, uint16_t color);
  ...
  private:    
    ...
    driver_rgb_t* driver_reg; //copy of configuration registers for drivers (initialization of the number of lines, brightness, etc.)
    uint8_t driver_reg_cnt; //number of configuration registers for the current driver
