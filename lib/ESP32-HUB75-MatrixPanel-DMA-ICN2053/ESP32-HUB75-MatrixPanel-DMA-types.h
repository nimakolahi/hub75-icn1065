...
typedef struct  HUB75_DMA_CFG_S{
  ...
  // For SHIFT drivers, double buffering is enabled by either double_buff or double_dma_buff.
  // For ICN2053 drivers, double_buff (primary video buffer) and double_dma_buff (output buffer) are independent.
  // Double DMA buffer (SHIFT) or double primary video buffer (ICN2053).
  ...
  bool decoder_INT595;
  // Physical panel dimensions for 1/8-scan remapping (physical 64x32 → DMA 128x16).
  // Set to 0 (default) to use DMA coordinates directly (no remap).
  uint16_t phys_width;
