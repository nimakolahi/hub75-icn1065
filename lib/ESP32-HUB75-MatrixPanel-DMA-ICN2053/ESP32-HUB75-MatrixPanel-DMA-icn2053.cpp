...
void MatrixPanel_DMA::panelShowOff()
{
  ...
  {
    // ICN1065 and SHIFT drivers do not support this command sequence yet
  }
}
...
IRAM_ATTR void MatrixPanel_DMA::sendCBVsync()
{
  ...
  driver_cur_reg++;
  if (driver_cur_reg >= driver_reg_cnt) driver_cur_reg = 0;
...
void MatrixPanel_DMA::icn2053init()
{
  ...
  for (int i = 0; i < driver_reg_cnt; i++)
  {
    sendVsync();
    //Serial.println(F("4"));
  }
...
bool MatrixPanel_DMA::allocateDMAmemory()
{
  ...
  */
  ...
  if ((m_cfg.driver == ICN2053) || (m_cfg.driver == ICN1065))
  { 
    bool is_icn1065 = (m_cfg.driver == ICN1065);
    ...
    if (is_icn1065)
    {
      dma_buff.frame_prefix_len = FRAME_ADD_LEN + ICN1065_PREFIX_START_LEN + pixels_per_row * 5;
      dma_buff.frame_prefix_cnt = ICN1065_PREFIX_CNT;
    }else
    {
    ...
    if (is_icn1065)
    {
      dma_buff.frame_suffix_len = ICN1065_ROW_OE_LEN * rows_per_frame;
      dma_buff.frame_suffix_cnt = ICN1065_SUFFIX_CNT;
    }else
    {
    ...
    dma_buff_cnt = dma_buff.frame_prefix_cnt + dma_buff.frame_suffix_cnt;
    matrix_buffer_size = (dma_buff.frame_prefix_len * dma_buff.frame_prefix_cnt +
                          dma_buff.frame_suffix_len * dma_buff.frame_suffix_cnt) * SIZE_DMA_TYPE;
  ...
  { 
    ...
    if (m_cfg.driver == ICN1065) desc_cnt *= ICN1065_DSUFFIX_CNT;
    else desc_cnt *= ICN2053_DSUFFIX_CNT;
  ...
  if ((m_cfg.driver == ICN2038) || (m_cfg.driver == ICN2053) || (m_cfg.driver == ICN1065))
  {
...
void MatrixPanel_DMA::configureDMA()
{
  ...
  if ((m_cfg.driver == ICN2053) || (m_cfg.driver == ICN1065))
  {
    ...
    //English: The driver registers are allocated in memory based on the selected driver type (ICN1065 or ICN2053). 
    //The number of registers is determined by the specific driver, and memory is allocated accordingly. 
    //If the allocation fails, the function frees any previously allocated buffers and exits. 
    //The driver registers are then initialized with predefined values, and specific configuration settings are applied based on the number of rows per frame. 
    //Finally, various flags and counters related to the driver state are initialized.
    if (is_icn1065) driver_reg_cnt = ICN1065_REG_CNT;
    else driver_reg_cnt = ICN2053_REG_CNT;
    driver_reg = (driver_rgb_t*)heap_caps_calloc(driver_reg_cnt, sizeof(driver_rgb_t), MALLOC_CAP_8BIT);
    if (driver_reg == NULL)
    {
      buffersFree();
      return;
    }
    // English: The driver registers are prepared in accordance with the selected driver and the number of rows per frame.
    // For the ICN1065, the scan configuration register is set to the number of rows per frame minus one, combined with a specific bit pattern.
    // For the ICN2053, a similar configuration is applied to the corresponding register using a helper function to set the RGB values appropriately.
    if (is_icn1065)
    {
      memcpy(driver_reg, ICN1065_REG_VALUE, ICN1065_REG_CNT * sizeof(driver_rgb_t));
      driver_reg[ICN1065_CFG_SCAN].r = 0x0200 | (rows_per_frame - 1);
      driver_reg[ICN1065_CFG_SCAN].g = driver_reg[ICN1065_CFG_SCAN].r;
      driver_reg[ICN1065_CFG_SCAN].b = driver_reg[ICN1065_CFG_SCAN].r;
    }else
    {
...
bool MatrixPanel_DMA::begin()
{
  ...
  if (m_cfg.phys_width && m_cfg.phys_height) {
    _width  = (int16_t)m_cfg.phys_width;
    _height = (int16_t)m_cfg.phys_height;
  }
